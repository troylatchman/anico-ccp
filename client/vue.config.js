...
const https = true; // set to true if app loads on a secured domain
const hostAndPort = "localhost:8080"; // a standard for all developers to use locally, do not change
const apiHostAndPort = "localhost:3000"; // a standard for all developers to use locally, do not change
let publicPath;
...
module.exports = {
  ...
  devServer: {
    ...
    proxy: {
      "^/ccp-api-local": {
        target: `http://${apiHostAndPort}`,
        pathRewrite: {
          "^/ccp-api-local": "/ccp-api"
        }
      }
    }
  }
